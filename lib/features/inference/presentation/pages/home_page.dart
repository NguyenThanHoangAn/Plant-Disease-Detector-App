...
class _DeepLearningHomePageState extends ConsumerState<DeepLearningHomePage> with WidgetsBindingObserver, AutomaticKeepAliveClientMixin {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      print('🔄 [HomePage] App resumed - State preserved: image=${_selectedImage?.path}');
    }
  }
  ...
  Future<void> _openInAppCamera() async {
    try {
      ...
      if (file != null && mounted) {
        print('🎯 [HomePage] Nhận ảnh từ CameraCapturePage: ${file.path}');

        setState(() {
          _selectedImage = file;
        });

        print('🧠 [HomePage] Bắt đầu phân tích ảnh...');
        await ref.read(inferenceProvider.notifier).runInference(file);
      } else {
        print('⚠️ [HomePage] User cancel');
      }
    } catch (e) {
      print('❌ [HomePage._openInAppCamera] Lỗi: $e');
      if (mounted) {
  ...
  Future<void> _openCameraPreview() async {
    try {
      ...
      if (file != null && mounted) {
        print('🎯 [HomePage] Nhận ảnh từ CameraPreviewPage: ${file.path}');
        ...
        print('⚠️ [HomePage] User cancel hoặc không chọn ảnh');
      }
    } catch (e) {
      print('❌ [HomePage._openCameraPreview] Lỗi: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Lỗi: $e'),
...
class _NavItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = isActive ? const Color(0xFF22C55E) : const Color(0xFF9CA3AF);
    return InkWell(
      onTap: onTap,
      borderRadius: BorderRadius.circular(12),
      child: Padding(
